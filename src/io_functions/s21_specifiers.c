...
void specifier_e(int *str_i, char *str, opt_struct *options, int big,
  ...
  if (options->precision) {  // если не округлилось до целого
    ...
    if (str[(*str_i - 2)] != '0' &&
        (options->specifier == 'g' || options->specifier == 'G'))
      options->precision--;
    for (int i = 0;
         i < ((options->specifier == 'g' || options->specifier == 'G')
                  ? options->precision
                  : 6);
         i++) {
      int num =
...
void specifier_f(int *str_i, char *str, opt_struct *options, long double d) {
  ...
  int non_significant_zeros = 0;
  int zeros_ahead = 0;
  for (long long int step = 10; ceil_part_d >= step; step *= 10)
    len_before_point++;
  if (options->specifier == 'g' || options->specifier == 'G')
    options->precision -= len_before_point;  // для спецификатора g
  double fraction = d - (long long int)d;  // дробная часть
  // Округление до целого
  if (options->precision == 0 && floor(fraction * 10 + 0.000000001) >= 5)
    ceil_part_d++;
  for (int i = len_before_point - 1; i >= 0; i--) {
    ...
    if (len_before_point == 1 && str[i + (*str_i)] == '0') {
      non_significant_zeros = 1;
      zeros_ahead = 1;
    }
    ceil_part_d /= 10;
  }
  ...
  if (options->precision) {  // если не округлилось до целого
    // Записываем дробную часть числа
    // int non_significant_zeros = 0;
    int end = options->precision;
    if (options->specifier == 'g' || options->specifier == 'G')
      end += non_significant_zeros;
    for (int i = 0; i < end; i++) {
      int num =
          (int)(fraction * 10 + 0.000000001);  // Костыль (0.1*10 = 0.99999999)
      str[i + (*str_i)] = (num) + '0';
      if (str[i + (*str_i)] == '0' && zeros_ahead &&
          (options->specifier == 'g' || options->specifier == 'G')) {
        non_significant_zeros++;
        end++;
      } else
        zeros_ahead = 0;
      fraction = 10 * fraction - num;
      ...
      // Округление последнего знака
      if ((i == end - 1) && (floor(fraction * 10 + 0.000000001) >= 5))
        str[i + (*str_i)] += 1;
      if (i == end - 1) break;
    }
    (*str_i) +=
        (end >= 6
             ? ((options->specifier == 'g' || options->specifier == 'G') ? end
                                                                         : 6)
             : end);  // Если точность > 6 двигаем на 6
...
void specifier_g(int *str_i, char *str, opt_struct *options, va_list *vl) {
  long double d_copy;
  opt_struct *options_copy = options;
  va_list *vl_copy = vl;
  init_arg_with_length(&d_copy, options_copy, vl_copy);

  long double d = d_copy;
  ...
  // Print the number using the chosen format
  if (use_decimal) {
    specifier_f(str_i, str, options, d_copy);
  } else {
    specifier_e(str_i, str, options, (options->specifier == 'G'), d_copy);
  }
}